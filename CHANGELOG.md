...
- **Enhanced Systems Integration**
  - Bundled ElvUI Enhanced features directly (disabled by default) for easy enablement
  - Minimap Button Grabber module with movable button grid, alpha/mouseover controls, and minimap anchoring
  - Action bar key press animations with configurable color, scale, and rotation
  - Animated achievement progress bars, tooltip icons, tooltip progression info, and item border color highlights
  - Mailbox QoL: Take All / Take Cash buttons, spam-protected automation, and recipient history
  - Error frame configuration (size, font, toggles) surfaced in-game
- **Zone Information Overlay**
  - Cromulent-style zone data overlay for Mapster (level ranges, instance info, fishing skill requirements, faction coloring)
  - New `LibTourist-3.0`, `LibBabble-Zone-3.0`, and Astrolabe integration for zone metadata
- **Trainer Quality of Life**
  - "Train All" button at class trainers with cost preview and safe retry logic
- **Totem Enhancements**
  - Totem bar borders now turn red when the associated spell is out of range, retaining element colors otherwise

### Improved
- Warmed up world map assets after login and expanded fog-clear pre-caching to remove first-open hitching
- Hardened chat GUID lookups, channel handling, and player link generation to avoid nil errors
- Deferred protected Mapster calls while in combat to prevent taint

### Technical
- Registered new libraries: `LibCompress`, `LibAnim`, `LibTourist-3.0`, `LibBabble-Zone-3.0`, and `Astrolabe`
- Added new saved variable `EnhancedDB` for the integrated modules
