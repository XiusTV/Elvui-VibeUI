...
function ItemRack.UpdateClassSpecificStuff()
	...
	if class=="SHAMAN" then
		ItemRack.CanWearOneHandOffHand = select(5,GetTalentInfo(2,20))>0 and 1 or nil
	end
end
...
function ItemRack.AnchorTooltip(owner)
	owner = owner or this
	-- Prevent tooltip from setting itself as owner
...
function ItemRack.SetSetBindings()
	...
	-- Wrap SaveBindings in pcall to prevent taint errors
	local success, err = pcall(SaveBindings, GetCurrentBindingSet())
	if not success then
		-- Silently fail - bindings will be temporary for this session
	end
end
