...
MBB_Ignore = {
	...
	[27] = "GameTimeFrame",
	-- Quest POI icons (prevent collection of quest markers ! and ?)
	[28] = "QuestPOI",
	[29] = "poiMinimap",
	[30] = "MiniMapQuestBlob",
	-- TomTom Arrow (prevent collection)
	[31] = "TomTomCrazyArrow",
	[32] = "CrazyArrow",
	[33] = "Arrow",
...
function MBB_PrepareButton(name)
	...
	buttonframe.Hide = function(...)
		...
		-- Call the original Hide function directly without recursion
		if innerframe.ohide then
			innerframe.ohide(innerframe);
		end
		innerframe.mbbHiding = false;
		if( not MBB_IsInArray(MBB_Exclude, innerframe:GetName()) ) then
			MBB_SetPositions();
		end
	end
